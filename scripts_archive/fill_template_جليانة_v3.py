...
try:
    ...
    df['nat_id_clean'] = df['الرقم_الوطني'].fillna('').astype(str).str.replace('.0', '', regex=False).str.strip()
    ...
    # Deduplicate: preferring principal if same card number exists for some reason
